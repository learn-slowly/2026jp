...
export function PolicySection({ candidate, settings }: PolicySectionProps) {
        ...
        <section id="policies" className="py-24 bg-gray-50">
            <div className="max-w-7xl mx-auto px-4 md:px-8">
                <div className="text-center max-w-3xl mx-auto mb-16">
                    <h2 className="text-gray-900 font-bold text-lg tracking-wider uppercase mb-4">{settings.section_policy_subtitle || '당신의 삶을 바꾸는 가장 확실한 방법'}</h2>
                    <h3 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6">
                        {candidate.name}의 약속
                    </h3>
                    <p className="text-xl text-gray-600">
                        {settings.section_policy_description || '더 나은 삶, 더 정의로운 사회를 위한 핵심 공약입니다.'}
                    </p>
                </div>
                    ...
                    {candidate.policies.map((policy, i) => (
                        <div key={i} className="bg-white rounded-2xl p-8 shadow-sm hover:shadow-xl transition-shadow border border-gray-100 group">
                            ...
                            <div className="flex items-center text-black font-medium text-sm mt-auto">
                                ...
                                {settings.section_policy_badge || '확실한 이행 약속'}
                            </div>
                        </div>
                    ))}
                </div>
